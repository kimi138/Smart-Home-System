...
public class Readlight 
{
    ...
    private static MyTimer3 myTimer;
	private int mutex = 0;
    ...
    class Read implements Runnable, SerialPortEventListener
    {
        ...
        class GetControl implements Runnable
        {
        	public void run() 
            {
        		...
        		try 
        		{
        			...
        			call.setOperationName(new QName("http://tempuri.org/", "getlcontrol")); 
        			call.setSOAPActionURI("http://tempuri.org/getlcontrol"); 
        			
        			while(true)
        			{
        				String retVal = (String) call.invoke(new Object[] {});
        				if(retVal.equals("light"))
        				{
        					System.out.println("打开照明电灯"); 
        				}
        				Thread.sleep(3000);
        			}
        		} 
        ...
        public Read(CommPortIdentifier portId) throws InterruptedException 
        {
			...
			call = (Call) service.createCall();
                            			...
                            			call.setReturnType(new QName("http://www.w3.org/2001/XMLSchema","string")); 
                            			// 第二种设置返回值类型为String的方法 
                            			call.setOperationName(new QName("http://tempuri.org/", "receivelight")); 
                            			call.setSOAPActionURI("http://tempuri.org/receivelight"); 
    ...
    {
        Readlight reader = new Readlight();
        //初始化定时器
        myTimer = new MyTimer3(1); 
        myTimer.start();
        reader.init();
        reader.retValue = "0";
    }
...
//在Readlightr 中定义一个静态变量static String str = "000000";
