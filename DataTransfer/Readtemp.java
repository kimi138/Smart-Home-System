...
public class Readtemp 
{
    ...
    private static MyTimer2 myTimer;
	private int mutex = 0;
        ...
        class GetControl implements Runnable
        {
        	public void run() 
            {
        		...
        		try 
        		{
        			...
        			call.setOperationName(new QName("http://tempuri.org/", "getthcontrol")); 
        			call.setSOAPActionURI("http://tempuri.org/getthcontrol"); 
        			
        			while(true)
        			{
        				String retVal = (String) call.invoke(new Object[] {});
                        
        				if(retVal.equals("cool"))
        				{
        					System.out.println("打开空调降温"); 
        				}
						if(retVal.equals("wet"))
						{
							System.out.println("打开加湿器加湿");
						}
        				Thread.sleep(3000);
        			}
        		} 
        ...
        public Read(CommPortIdentifier portId) throws InterruptedException 
        {
			try
			{
				call = (Call) service.createCall();
                            			...
                            			call.setReturnType(new QName("http://www.w3.org/2001/XMLSchema","string")); 
                            			// 第二种设置返回值类型为String的方法 
                            			call.setOperationName(new QName("http://tempuri.org/", "receivet")); 
                            			call.setSOAPActionURI("http://tempuri.org/receivet"); 
                        ...
                        while (inputStream.available() > 0) 
                        {
                            ...
                            if(temps.length>3)
                            {
                            	...
                            	if(myTimer.counter == 5 && mutex == 0)
                            	{
                            		...
                            		try 
                            		{
                            			if(Float.parseFloat(temps[1].trim()) - prev_temp >= 1.0f || prev_humid - Float.parseFloat(temps[3].trim()) >= 1.0f)
                                        {
                            			     String retVal = (String) call.invoke(new Object[] { temps[1].trim()+"$" + temps[3].trim()+"$" + df.format(new Date())});
                            			     System.out.println(retVal); 
                                             prev_temp = Float.parseFloat(temps[1].trim());
                                             prev_humid = Float.parseFloat(temps[3].trim());
                                        }
                            		} 
                            		catch (Exception e)
                            		{
    ...
    {
        Readtemp reader = new Readtemp();
        //初始化定时器
        myTimer = new MyTimer2(1); 
        myTimer.start();
...
//在Readtempr 中定义一个静态变量static String str = "000000";
